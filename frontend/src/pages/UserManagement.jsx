  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-background text-slate-900 dark:text-foreground overflow-y-auto relative">
      ...
      <div className="max-w-7xl mx-auto w-full p-6 md:p-10 relative z-10 flex-1 flex flex-col">
        ...
        {/* Users Table */}
        <div className="bg-white/[0.02] border border-slate-200 dark:border-border rounded-3xl overflow-hidden shadow-2xl">
              ...
              <tbody className="divide-y divide-white/5 text-xs">
                {filteredUsers.map(u => (
                  <tr key={u.id} className="hover:bg-white/[0.02] transition-colors">
                    <td className="py-4 px-6 font-bold text-slate-900 dark:text-foreground flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-secondary border border-slate-200 dark:border-border flex items-center justify-center font-bold text-slate-700 dark:text-muted-foreground">
                        {u.name.charAt(0)}
                      </div>
                      {u.name}
                    </td>
                    <td className="py-4 px-6 text-slate-700 dark:text-muted-foreground font-mono">{u.email}</td>
                    <td className="py-4 px-6 text-slate-700 dark:text-muted-foreground font-semibold">{u.department}</td>
                    ...
                    <td className="py-4 px-6 text-slate-700 dark:text-muted-foreground">{u.joined}</td>
                    <td className="py-4 px-6 text-right">
                      <div className="flex justify-end gap-2">
                        <button className="p-2 rounded-lg bg-slate-100 dark:bg-secondary hover:bg-white/10 text-slate-700 dark:text-muted-foreground transition-colors">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button 
                          onClick={() => handleDeleteUser(u.id)}
                          className="p-2 rounded-lg bg-slate-100 dark:bg-secondary hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 transition-colors"
                        >
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2.5 bg-slate-100 dark:bg-secondary border border-slate-200 dark:border-border rounded-xl text-xs font-semibold focus:outline-none focus:border-purple-500/50 text-slate-900 dark:text-foreground"
                      />
                    </div>
                    <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <select 
                        ...
                        className="w-full px-3 py-2.5 bg-[#0a0f1d] border border-slate-200 dark:border-border rounded-xl text-xs font-semibold focus:outline-none focus:border-purple-500/50 text-slate-900 dark:text-foreground"
                      >
