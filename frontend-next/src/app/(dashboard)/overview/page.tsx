...
function CircularProgress({ value, label, sublabel, color, trackColor }: any) {
  const radius = 45;
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-slate-500 font-semibold mt-1 uppercase tracking-widest">{sublabel}</p>
    </motion.div>
  );
}
...
export default function Overview() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 md:p-10 text-white relative h-full">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            <h1 className="text-4xl font-black tracking-tight bg-gradient-to-r from-white via-indigo-100 to-indigo-200 bg-clip-text text-transparent font-outfit">Enterprise Analytics</h1>
            <p className="text-sm text-slate-400 font-semibold mt-2 uppercase tracking-widest">Real-time LLM tracking & system health</p>
          </div>
          <div className="flex flex-wrap items-center gap-4">
            <motion.button 
              ...
              className="px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white text-sm font-black rounded-xl shadow-[0_0_30px_rgba(79,70,229,0.5)] transition-all flex items-center gap-3 border border-white/10"
            >
            ...
            >
              Start Chat <ArrowRight className="w-5 h-5 text-indigo-400" />
            </motion.button>
          </div>
        </motion.div>
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: "Queries Processed", value: stats.query_count.toLocaleString(), icon: Activity, color: "#818cf8", bg: "from-indigo-500/20 to-transparent" },
        ...
        <motion.div variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          >
            ...
            <h3 className="text-[13px] font-black text-slate-400 uppercase tracking-widest mb-8 flex items-center gap-2">
              <Activity className="w-4 h-4 text-indigo-400" /> Interaction Volume (7 Days)
            </h3>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.volume_history.length ? stats.volume_history : areaData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    labelStyle={{ color: '#94a3b8', fontSize: '12px', fontWeight: 'bold', marginBottom: '8px' }}
                  />
                  <Legend verticalAlign="top" height={36} iconType="circle" wrapperStyle={{ fontWeight: 'bold', color: '#94a3b8', fontSize: '12px' }} />
                  ...
                  <Area type="monotone" name="User Queries" dataKey="queries" stroke="#818cf8" strokeWidth={4} fillOpacity={1} fill="url(#colorQueries)" activeDot={{ r: 8, fill: "#818cf8", stroke: "#fff", strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <CircularProgress value={stats.hallucination_rate === '0%' ? 100 : 100 - parseFloat(stats.hallucination_rate)} label="Avg Accuracy" sublabel="Confidence Score" color="#818cf8" trackColor="#818cf820" />
            </div>
          </div>
        </motion.div>
      </motion.div>
